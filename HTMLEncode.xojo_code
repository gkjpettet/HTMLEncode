...
Protected Module HTMLEncode
	#tag Method, Flags = &h21
		Private Function CharToSymbol(char as Text) As Text
		  ' Checks to see if `char` is an entity character. If so, we return the symbol. If not, we return `char`.
		  
		  if characters.HasKey(char) then
		    return characters.Value(char)
		  else
		    return char
		  end if
		End Function
	...
	#tag Method, Flags = &h1
		Protected Function DecodeEntities(Extends t as Text) As Text
		  ' Convenience method
	...
	#tag Method, Flags = &h1
		Protected Function DecodeEntities(t as Text) As Text
		  // TODO
		  
		  ' Replace any HTML encoded symbols with their corresponding character. Entity symbols are flanked
		  ' with `&` and `;` so we can use a simple RegEx to find them.
		  ...
		  rg = new RegEx
		  rg.SearchPattern = ENTITY_SYMBOLS_REGEX
		  
		  ' Run the RegEx searches
		  match = rg.Search(t)
		  while match <> Nil
		    ' For this match, get the symbol
		    symbol = match.SubExpressionString(1).ToText
		    
		    if symbols.HasKey(symbol) then
		      t = t.Replace(match.SubExpressionString(0).ToText, symbols.Value(symbol))
		    end if
		...
		Private Sub Initialise()
		  ...
		  if symbols = Nil then 
		    ...
		    for each entry as DictionaryEntry in characters
		      ...
		      symbols.Value(entry.Value) = entry.Key
	...
	#tag Property, Flags = &h21
		...
		#tag EndNote
		Private characters As xojo.Core.Dictionary
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		...
		#tag EndNote
		Private symbols As xojo.Core.Dictionary
	#tag EndProperty
